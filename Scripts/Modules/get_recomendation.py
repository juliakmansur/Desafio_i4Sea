...
class RecomendadorOperacional:
    def __init__(self, base_model_path: str):
        '''
        base_model_path: caminho onde os modelos .pkl estão salvos
        As variáveis de entrada são automaticamente recuperadas do nome do modelo.
        '''
        self.base_model_path = base_model_path
        self.variaveis_por_cenario = self._carregar_variaveis_por_cenario()

    def _carregar_variaveis_por_cenario(self) -> dict:
        '''Recupera variáveis usadas em cada cenário a partir de arquivos auxiliares salvos (ex: .csv por modelo)'''
        variaveis = {}
    ...
    def _find_closest_point(self, ds, lat_target, lon_target):
        lat = ds['lat'].values
        lon = ds['lon'].values
    ...
    def prever(self, path_nc, cenario, modelo, lat=None, lon=None):
        key = f"{modelo.lower()}_{cenario}"
        ...
        df = df[['time'] + variaveis_originais + variaveis_bin]
...
class RecomendadorVisual:
    ...
    def gerar_relatorio(self, resultado, modelo, cenario, nome_arquivo=None, salvar_em=None):
        ...
        fig.suptitle(f'Série Temporal para Paradas Recomendadas ({nome_amigavel} - {cenario.capitalize()})', fontsize=16, y=.94)
        ...
        if salvar_em and nome_arquivo:
            ...
            consolidado = grupos.agg(Incio_da_Parada=('time', 'min'), Fim_da_Parada=('time', 'max')).reset_index(drop=True)
            ...
            print(f'Relatório salvo em: {fig_path}')
        else:
            plt.show()
